...
void setup() {
  AFMS.begin(50);
  Serial.begin(115200);
  Serial.println("System Ready. Send: Enable,Dir,Turn,Speed (e.g. 1,90,0,50)");
...
void calculateMecanum(int degrees, int turn_val, int speed_val) {
  ...
  // Calculate X and Y components from degrees
  // 90 = Forward, 180 = Left, 270 = Backward, 0/360 = Right
  float rad = (degrees * PI) / 180.0;
  x = cos(rad) * targetSpeedRPM;
  y = sin(rad) * targetSpeedRPM;

  // Handle Turning
  if (turn_val == 1) t = -targetSpeedRPM;      // Spin Left
  else if (turn_val == 2) t = targetSpeedRPM;  // Spin Right

  // Mecanum Kinematics
  targetRPMs[1] = x + y + t;  // Front Left
  targetRPMs[0] = x - y - t;  // Front Right
  targetRPMs[2] = x - y + t;  // Rear Left
  targetRPMs[3] = x + y - t;  // Rear Right

  // Scaling to ensure we don't exceed MAX_RPM_LIMIT
  float maxVal = 0;
  for (int i = 0; i < 4; i++) {
